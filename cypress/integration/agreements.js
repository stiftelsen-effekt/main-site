describe("Details page", () => {
  ...
  it("Should display a list of active and inactive agreements", () => {
    ...
    cy.get("[data-cy=generic-list-table]").first().find("tbody").first().should("contain.text", "Vipps")
    cy.get("[data-cy=generic-list-table]").first().find("tbody").first().should("contain.text", "Den 10. hver måned")
    cy.get("[data-cy=generic-list-table]").first().find("tbody").first().should("contain.text", "100 kr")
    cy.get("[data-cy=generic-list-table]").first().find("tbody").first().should("contain.text", "95231965")

    cy.get("[data-cy=generic-list-table]").last().find("tbody").first().should("contain.text", "Vipps")
    cy.get("[data-cy=generic-list-table]").last().find("tbody").first().should("contain.text", "Den 11. hver måned")
    cy.get("[data-cy=generic-list-table]").last().find("tbody").first().should("contain.text", "400 kr")
    cy.get("[data-cy=generic-list-table]").last().find("tbody").first().should("contain.text", "16753477")
  })
  ...
  it("Should be possible to change the agreement distribution", () => {
    ...
    cy.fixture('vipps').then((vipps) => {
      cy.intercept("GET", "/donors/*/recurring/vipps/", { 
        ...
        body: {
          status: 200,
          content: [...vipps.filter(vipps => vipps.ID != "agr_58LgZZM"), {
            "ID": "agr_58LgZZM",
            "status": "ACTIVE",
            "donorID": 27,
            "full_name": "Keef",
    ...
    cy.fixture('kids_agreements').then((kids) => {
      cy.intercept("GET", "/donors/*/distributions/*", { 
        ...
        body: {
          status: 200,
          content: [...kids.filter(dist => dist.kid != "95231965"), {
            "kid": newKid,
            "organizations": [
                {
                    "id": 12,
  ...
  it("Should move agreement to inaktive list when cancelling agreement", () => {
    ...
    cy.get("[data-cy=lightbox-confirm]")
      .click()
