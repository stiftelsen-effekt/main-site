...
function MyApp({
  Component,
  pageProps: { appStaticProps, ...pageProps },
}: AppProps<{
  ...
  const routerContextValue = useRef<RouterContextValue>(appStaticProps.routerContext);
  ...
  const { data: previewData, loading: previewLoading } = usePreviewSubscription(
    pageProps.data?.query || "",
    {
      ...
      enabled: !!pageProps.data && appStaticProps.preview,
    },
  );
  ...
  if (previewData?.page && Array.isArray(previewData.page)) {
    previewData.page = filterPageToSingleItem(previewData, appStaticProps.preview);
  }
