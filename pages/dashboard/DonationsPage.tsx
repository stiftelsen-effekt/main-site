...
export const DonationsPage = withStaticProps(
  ...
  if (noDonationData) {
    const emptyDonationList = tableConfiguration ? (
      <DonationList
        donations={[]}
        distributions={new Map<string, Distribution>()}
        taxUnits={taxUnits}
        year={page.year_menu_total_title}
    ...
    return (
      <>
        ...
        <PageContent>
          ...
          <DonationYearMenu totalTitle={page.year_menu_total_title} years={[]} selected={"total"} />
...
const getYears = (donor: Donor, donations: Donation[]) => {
  ...
  const donationYears = donations.map((donation) => new Date(donation.timestamp).getFullYear());
  const minimumDonationYear =
    donationYears.length > 0 ? Math.min(...donationYears) : registeredYear;
